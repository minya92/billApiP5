...
QUnit.test("Пополнить счет на 1300", function (assert) {
    var done = assert.async();
    request("POST", "operations/create", {id: accountId, sum: 1300}, function (res) {
        assert.ok(res.id, "Operation Processing: " + res.id + errorMsg(res));
        request("POST", "operations/done", {id: res.id}, function (r) {
...
QUnit.test("Снять со счета 901 (не хватит денег)", function (assert) {
    var done = assert.async();
    request("POST", "operations/create", {id: accountId, sum: 901, withdraw: true}, function (res) {
        assert.ok(res.id, "Operation Processing: " + res.id + errorMsg(res));
        request("POST", "operations/done", {id: res.id}, function (r) {
            assert.ok(!r.result, "Operation done: " + r.result + errorMsg(r));
            done();
...
QUnit.test("Создание услуги с предоплатой (стоимость 50)", function (assert) {
    var done = assert.async();
    request("POST", "services/create", {name: "test service with prepayment", cost: 50, days: 7, prepayment: true}, function (res) {
...
QUnit.test("Снять со счета стоимость равную УСЛУГЕ С ПРЕДОПЛАТОЙ (50)", function (assert) {
    var done = assert.async();
    request("POST", "operations/create", {id: accountId, service_id: servicePrepayment}, function (res) {
        assert.ok(res.id, "Operation Processing: " + res.id + errorMsg(res));
        request("POST", "operations/done", {id: res.id}, function (r) {
            assert.ok(r.result, "Operation done: " + r.result + errorMsg(r));
            done();
...
QUnit.test("Создание услуги (стоимость 500, разовое списание)", function (assert) {
    var done = assert.async();
    request("POST", "services/create", {name: "test service", description: "Description", cost: 500, once: true}, function (res) {
...
QUnit.test("Изменить услугу + проверка на одноразовость ", function (assert) {
    var done = assert.async();
    request("POST", "services/change", {name: 'New serviceName', description: 'New Description', service_id: serviceId}, function (res) {
        console.log(res);
        assert.equal(res.service_id, serviceId, "Тот ли ID: " + JSON.stringify(res.service_id) + errorMsg(res));
        assert.equal(res.service_name, 'New serviceName', "Изменилось ли имя: " + JSON.stringify(res.service_name) + errorMsg(res));
        assert.equal(res.service_desc, 'New Description', "Изменилось ли описание: " + JSON.stringify(res.service_desc) + errorMsg(res));
        assert.ok(res.service_type_id, "Есть ли тип: " + JSON.stringify(res.service_type_id) + errorMsg(res));
        done();
    });
});

QUnit.test("Создание услуги со счетчиком (стоимость 101) с переносом средств", function (assert) {
    var done = assert.async();
    request("POST", "services/create", {name: "service counter!!!", cost: 101, counts: 5, type: 'CounterServiceModule', days: 1, transfer: true}, function (res) {
...
QUnit.test("Создание услуги с револьвацией (предоплата) (стоимость 300)", function (assert) {
    var done = assert.async();
    request("POST", "services/create", {name: "service revaluation", cost: 300, prepayment: true, rev: true, revDays: 1, revTransfer: true, revCounts: 1}, function (res) {
        serviceRev = res.service_id;
        assert.ok(res.service_id, "RESULT: " + JSON.stringify(res.service_id) + errorMsg(res));
        done();
    });
});
...
var serviceAccountRev = null; // подкл. услуга с револьвацией

QUnit.test("Подключить услугу со счетчиком на счет и поетстить счетчик", function (assert) {
    var done = assert.async();
...
QUnit.test("Снять со счета 1 по обычной услуге (serviceId) и уменьшить counter", function (assert) {
    var done = assert.async();
    request("POST", "operations/create", {id: accountId, sum: 1, withdraw: true, service_id: serviceId}, function (res) {
        assert.ok(res.id, "Operation Processing: " + res.id + errorMsg(res));
        request("POST", "operations/done", {id: res.id}, function (r) {
            assert.ok(r.result, "Operation done: " + r.result + errorMsg(r));
            request("POST", "services/on_account", {service_account_id: serviceAccountCounter}, function (service) {
                assert.equal(service.result[0].service_counts, 3, "Counts limit on this Service = 3; Errors:" + errorMsg(service));
                done();
            });
        });
    });
...
QUnit.test("Обработать все запланированные операции на сегодня и на завтра", function (assert) {
    var done = assert.async();
    var date = new Date();
    date.setDate(date.getDate() + 1);
    date = (date.getMonth() + 1) + '.' + date.getDate() + '.' + date.getFullYear();
    console.log("cron/planned/" + date);
    request("POST", "cron/planned/" + date, {}, function (res) {
        console.log(res);
...
QUnit.test("Баланс счетчика", function( assert ) {
    ...
    request("POST", "services/on_account", {service_account_id: serviceAccountCounter}, function(service){
        console.log(service);
        assert.equal(service.result[0].service_counts, 8, "Counts limit on this Service = 3; Errors:" + errorMsg(service));
        done();
    });
});
...
QUnit.test("Подключить услугу с револьвацией на счет", function (assert) {
    var done = assert.async();
    console.log("!!!!!!!!!!!!!!=" + serviceRev);
    request("POST", "services/add", {service_id: serviceRev, account_id: accountId}, function (res) {
        serviceAccountRev = res.result;
        assert.ok(res.result, "RESULT: " + JSON.stringify(res.result) + errorMsg(res));
        console.log(serviceAccountRev);
        request("POST", "services/on_account", {service_account_id: serviceAccountRev}, function (service) {
            console.log(service);
            assert.equal(service.result[0].service_counts, 1, "Counts limit on this Service = 1; Errors:" + errorMsg(service));
            request("POST", "services/set_count", {service_account_id: serviceAccountRev}, function (service2) {
                console.log(service2);
                assert.equal(service2.result[0].service_counts, 0, "-1 count. Counts limit on this Service = 0; Errors:" + errorMsg(service2));
                request("POST", "services/set_count", {service_account_id: serviceAccountRev}, function (service2) {
                    console.log('!РЕВОЛЬВАЦИЯ', service2);
                    assert.ok(service2.error, "Ошибка списания.:" + errorMsg(service2));
                    done();
                });
            });
        });
...
QUnit.test("Все услуги на счету", function (assert) {
    ...
    request("POST", "services/on_account", {account_id: accountId}, function (res) {
        assert.equal(res.result.length, 4, "Спсиок всех услуг: " + JSON.stringify(res.result) + errorMsg(res));
...
QUnit.test("Все операции по счету", function (assert) {
    ...
    request("POST", "operations/get", {id: accountId}, function (res) {
        assert.equal(res.result.length, 14, "RESULT: " + JSON.stringify(res.result) + errorMsg(res));
        done();
    });
});
...
QUnit.test("Все пройденные операции по счету", function (assert) {
    ...
    request("POST", "operations/get", {id: accountId, status: 'done'}, function (res) {
        assert.equal(res.result.length, 7, "RESULT: " + JSON.stringify(res.result) + errorMsg(res));
        done();
    });
});
...
QUnit.test("Запланированные операции по счету", function (assert) {
    ...
    request("POST", "operations/get", {id: accountId, status: 'planned'}, function (res) {
        assert.equal(res.result.length, 5, "RESULT: " + JSON.stringify(res.result) + errorMsg(res));
        done();
    });
});
...
QUnit.test("Пройденные операции списания по счету", function (assert) {
    ...
    request("POST", "operations/get", {id: accountId, type: 'withdraw', status: 'done'}, function (res) {
        assert.equal(res.result.length, 6, "RESULT: " + JSON.stringify(res.result) + errorMsg(res));
        done();
    });
});
...
QUnit.test( "Удалить обычную услугу (получится, хотя она подключена на счет)", function( assert ) {
    ...
    request("POST", "services/delete", {service_id: serviceId}, function(res){
        ...
        request("POST", "services/on_account", {account_id: accountId}, function (res) {
            assert.equal(res.result.length, 4, "Сколько услуг на аккаунте: " + JSON.stringify(res.result) + errorMsg(res));
            request("POST", "services/get", {}, function (res) {
                var match = false;
...
QUnit.test("Получить остаток средств на счете", function (assert) {
    ...
    request("POST", "accounts/get_sum", {id: accountId}, function (res) {
        assert.equal(res.sum, 399, "Sum: " + res.sum + errorMsg(res));
        done();
    });
});
...
QUnit.test("Удалить услугу с предоплатой (отключить) и отписать со всех счетов", function (assert) {
    ...
    request("POST", "services/delete", {service_id: servicePrepayment, unsubscribe: true}, function (res) {
        console.log('!!!!Удаление', res);
        assert.ok(true, "Message: " + res.result + errorMsg(res));
        request("POST", "services/on_account", {account_id: accountId}, function (res) {
            console.log('!!!!!!!!', res);
            assert.equal(res.result.length, 2, "Проверка отписалась ли она со счета: " + JSON.stringify(res.result) + errorMsg(res));
            done();
        });
    });
});
...
QUnit.test("Полностью удалить услугу с револьвацией", function (assert) {
    var done = assert.async();
    request("POST", "services/delete", {service_id: serviceRev, unsubscribe: true}, function (res) {
        request("POST", "services/hard_delete", {service_id: serviceRev}, function (res) {
            assert.ok(res.result, "Message: " + res.result + errorMsg(res));
            done();
        });
    });
